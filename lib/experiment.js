...
Experiment.prototype.createCampaign = function(client) {
    //find the project - assume it's one directory above
    var project = new Project({}, path.normalize(this.baseDir + "/.."));
    project.loadFromFile();
    ...
    var theClient = client;
    var self = this;
    if (this.attributes.campaign_id) {
        logger.log("info", "campaign already exists, skipping");
        return Promise.resolve(client);
    } else {
        logger.log("info", "creating remote campaign");
    
        var campaignArgs = {
            'project_id' : Number(project.attributes.id),
            'name' : 'Test Campaign',
            'page_ids' : [
                Number(self.attributes.page_id)
            ]
        };
        
        console.log(JSON.stringify(campaignArgs));
        
        return client.createCampaign({'action':'publish'}, campaignArgs)
        .then(function(data){
            self.attributes.campaign_id = data.payload.id;
            self.saveAttributes();
            return theClient;
        });
    }
...
Experiment.prototype.createRemote = function(client) {
  ...
  return this.createPage(client).then(function(client) {
      ...
      return self.createCampaign(client).then(function(client) {
          ...
          var expArgs = {
              ...
              "campaign_id" : self.attributes.campaign_id,
              "type": "a/b",
              "variations" : [],
              "changes" : [
                {
